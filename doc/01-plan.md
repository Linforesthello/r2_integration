...
>
> **史档声明（2026-09-09 加注）**：本文件 = 规划总纲 + 早期执行史。正文（尤 §6.3/§7.3/附录 A 命令块）
> 为写作时快照，**部分命令/状态已过期**（如早期 G354 启动缺 `mount_axes:=y_front_x_left_z_down`、
> 启动方式随包演进）。**现行启动命令以 [startup.md](startup.md) 为准**（全栈唯一权威，09-04 起）；
> 进度状态以 [02-progress.md](02-progress.md)/[07-handover.md](07-handover.md) 为准。规划章节不受影响。
...
在 Phase 2 已建图的基础上，让 R2 实现自主导航。VLP16 提供 360° 环境感知用于避障，AMCL（激光+地图匹配）定位，Nav2 规划控制。
**架构一句话**：3D 眼睛（VLP-16 点云）+ 3D 大脑（6DOF 里程计）做感知，2D 决策（Nav2）做导航——地面全向车（3 自由度 x,y,θ）的正确架构。

### 8.2 当前状态 → 目标

```
当前：✅ 首闭环跑通（2026-08-15，详见 retrospect/2026-08-15_nav2_bringup.md）
...
目标（下一阶段，分三层次推进）：
  ...
  ② 感知升级（中期）：定位精度升级 KISS-ICP → FAST-LIO2 评估
     ...
     - FAST-LIO2 = LiDAR-IMU 紧耦合 6DOF 里程计，接 R2 的 G354 IMU
     - 已知障碍：ROS2 分支硬依赖 Livox 消息/回调，VLP-16 需适配（见第七章历史注记）
     - 路线：VM 编译验证 → 与 KISS 两路里程计实车对比 → 数据说话再决定
     - 注意：升级的是"感知/里程计"，导航决策仍是 2D Nav2

  ③ 长期：真 3D 能力按需引入
     - 只有出现越障/爬坡/机械臂抓取等需求时，才引入对应 3D 规划栈
     - 不为 3D 而 3D（当前架构已是地面车的正确形态）
```
...
> 注：FAST-LIO2 → octomap **3D 导航**方案曾否决（ROS2 分支依赖 Livox，见第七章历史注记）；
> 08-15 起 FAST-LIO2 作为**感知升级方向**重新评估（LiDAR-IMU 紧耦合里程计替代 KISS-ICP，决策仍是 2D Nav2），见 [8.2 下一阶段目标](#82-当前状态--目标)。
...
| FAST-LIO2 | Fast LiDAR-Inertial Odometry | LiDAR-IMU 紧耦合 SLAM（感知升级候选，VLP-16 适配验证通过 08-18，接 G354 解决旋转漂移） |

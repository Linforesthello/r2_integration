# 单 3D 雷达高置 × 平面导航：低矮障碍感知手段调研清单（2026-09-05，09-06 实车更新）

> 状态：调研清单（09-05 WebSearch 核实，全站链接可回查）→ **09-06 手段 ②④ 已实车实施验证**
> （修法 B/A：点云低带直进 costmap + 双源双层），**远距有效、近距物理边界实锤**（实施结果与
> L1-L4 分层认知见 §三B）→ **09-08 收手注记**：修法 B 实车第一次接近有效，但二次失效根因实锤 =
> 盲区保留 mark 被 global 整层清空事件一票抹除且不可再生（结构性冲突）→ **09-08 用户决策：
> 低物机制精修收手，主线转 3D 规控**，本清单手段方向存留作阶段二+/加传感器时参考 —
> [决策 09-08](../retrospect/2026-09-08_lowobstacle_pivot_decision.md)、
> [根因 09-08](../retrospect/2026-09-08_lowobstacle_secondfail_clearevent.md)（细节不在此复制，单一事实来源）
> 仍标注「未实测」处不可当已验证结论（standards §1.11）
> 定位：为「低物盲区修法」提供**社区/工业做法方向池**——复盘 09-04 裁定修法后置（阶段二候选）
> 已被 09-05/09-06 实车链改写（实施 + 新边界），本文承载手段结论升级
> 关联：[复盘 09-04 §六-3/§10.4](../retrospect/2026-09-04_lowobstacle_breakpoint.md)（断点与修法候选）、
> [retrospect 09-06 §11](../retrospect/2026-09-06_lowobstacle_fixB_crashbox.md)（事件复盘 + 整体审视）、
...
| 手段 | 实施 | 实测结果 |
|:--|:--|:--|
| ② costmap 吃 points（velodyne_low 低带源） | ✅ local（09-05）+ global（09-06 e87a3f8） | **远距有效**：0.35m 箱 1.6m 外 local/global 均刷出 254 黑块 |
| ④ 高/低双源双层 | ✅ /scan 高带管墙定位 + velodyne_low 低带管 costmap | 结构正确但**同层 clear 冲突未隔离**（见下） |
| ① 转换层合成 scan | 未做（被 ② 取代，更优） | — |
| ⑤ 行为层 | 未做 | **当前最大缺口**（近距无兜底） |

### 三B.2 物理边界实锤（L1 层——手段 ② 的收益尽头）

- VLP-16 高置 0.775m + 最低环 -15° → 0.35m 箱顶打顶临界 **d=(0.655−0.23)/tan15°≈1.59m**；
  **d<1.59m 全部 16 环掠顶，低带源无 mark 输入**（09-06 bag 定点曲线：254 随逼近 1.9m→1.4m 单调归零）
- 即：手段 ② 解决"2D 化选环"问题，**解决不了"传感器没照到"**——近距低物信息对高置单雷达
  在几何上不存在；0.6m 近障同类（入带需 >0.95m，且更近全掠顶）
- 手段 ④ 实施中还暴露 **L3 语义冲突**：ObstacleLayer 同层 scan 2D raytrace clear 会清 velodyne_low
  mark（层内 clear 无差别）→ 需独立 mark-only 层隔离（nav2 多层结构，候选）
- 本节数值为**唯一事实源**；其**判据/对账方法**（视锥临界公式、理论↔实测 <1cm 对账门槛、反向选型用法）
  已规则化 → [ros2-ops.md §12](../ros2-ops.md)「感知覆盖 / 可观测性判据」

### 三B.3 L1-L4 分层认知（2026-09-06 系统性复盘，镜像 retrospect 09-06 §11）

| 层 | 问题 | 状态 |
|:--|:--|:--|
| L1 物理/布局 | 近距低物视锥几何盲区 | 未修（改布局/多模态，roadmap §3 线） |
| L2 转换层 | /scan 单环结构盲 | ✅ 已绕过（低带源） |
| L3 costmap 层 | 同层 clear 冲突 / local-global 不对称 / update 节流 0.64Hz | 部分修（clear 冲突待隔离；节流待查） |
| L4 规划/行为层 | 无近距兜底（几何盲区锥限速/停车策略） | 未修——**推荐近期补**（fail-safe，不依赖感知完美） |
...
5. **AMCL 边界**：AMCL 只吃单一 scan（2.3），若低带 scan 用于定位需先合流——R2 现状
   （AMCL 用现有 /scan）无此需求，仅记录防误用。
6. 物理布局/多模态（L1，D435 前视 / MID-70 A/B / 降装）挂 roadmap §3 传感器线，阶段二后考虑。

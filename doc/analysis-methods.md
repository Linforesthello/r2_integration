...
## 主题 F：批量预盘点流程（全 doc 检索 → 分桶呈报 → 双批落地）

> 来源事件：[09-09 A1 预盘点](retrospect/2026-09-09_experience_preinventory.md)（7 批全 doc 检索 → 126 候选 → 逐桶批准 → 双批落地）。
> 候选去向：成熟后抽入 [doc-engineering.md](doc-engineering.md)（盘点/收口流程邻域）；单事件新经验先落此（§十二 原则 2）。

### F1 批次切分：按「时间块 + 目录域」，底层源文件只读

- 切批依据 = 自然归档边界（retrospect 按时间块、minimal-loop/phaseN/n97 按目录、主线层单设）；单批源文档 ≤~15 篇为宜
- 底层源文件（retrospect 事件等）**只读不拆不动**——盘点不改变事件层叙事
- 主线层（README/01~03/07/startup/pending/roadmaps/surveys）单独成批做交叉核缺（见 F5）

### F2 批内输出格式统一：候选六列表 + 两段防重

- 候选表列 = 编号 / 候选 / 类型 / 来源 / 建议去向 / 备注（批内编号唯一，供分桶引用）
- 每批附两段防重复报告：**「已规则化核对（不再报）」**（列已入规范的同源条目）+ **「零增量确认」**（判重过的零候选域）——
  否则同一经验在多批/多轮被反复呈报，信噪比崩
- 弱候选照登（标「弱」+ 备注），不自行丢弃——搁置/降级本身就是留痕

### F3 分桶呈报与编号桶批准：批准前不动文件

- 全批返回后**汇总分桶**（按主题并入编号桶），一次性呈报用户；**批准前不落盘任何文件**
- 用户按**桶编号**回执批准（09-09 实路径：`⑥⑨①④③②⑦⑤ → 架构项 → ⑧`）
- 「待用户核实」类（批内交叉矛盾）单列附于呈报，不混入批准桶

### F4 落地二分 commit：经验回写 vs 结构修复

- 批准的桶按**类型二分落地**：经验/规则回写（只增不删 + 来源回指）与结构冲突修复（陈旧同步/双写收敛）**分两批 commit**——
  审阅面清晰、可独立回滚（先例：247e8b8 经验 9 文件 / d827e0b 结构 15 文件）
- commit body 三要素（改动/原因/影响）+ 事件档回指（standards §1.10）

### F5 交叉核缺独立成批：承载文档自洽 ≠ 全局同步

- 单文件批（A~F）只能看到「该文件自洽」；看板/索引/引用滞后只在**主线层专批**暴露
  （09-09 G 批查出看板停更、knowledge-tree 参数失新、survey 开放点未勾销等 7 条）
- 该批产出的「收手事件状态板同步卡」已规则化入 doc-engineering §十一

### F6 底稿与收口：处置先约定，登记不可漏

- 中间底稿落 `raw_data/`（不入 git）+ 文末「执行落账」节（批次 → 批准 → commit 轨迹）
- **底稿处置（升格/清理）当场约定**——09-09 悬置（教训 E6）；升格 = 事件档（叙事/轨迹）+ 本主题（流程）
- 收口三步不可漏：retrospect/README 登记 + 根 README 树同步 + 校验器（本次致坏 = 0）
...
| F1 批次切分 | 沉淀 | doc-engineering（盘点流程） | 复用 1+ 次或盘点抽取 |
| F2 六列表+防重两段 | 沉淀 | 同上 | 同上 |
| F3 编号桶批准 | 沉淀 | 同上 | 同上 |
| F4 落地二分 | 沉淀 | 同上 | 同上 |
| F5 核缺独立批 | 沉淀 | 同上（部分已入 §十一 核对卡） | 同上 |
| F6 底稿处置约定 | 沉淀 | 同上 | 同上 |

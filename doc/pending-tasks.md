...
- **~~近距丢黑块修复~~（低物链收手定论 09-07）**：修复 = `nav2_params_low.yaml` velodyne_low 拆独立 mark-only 层（**commit 180077c 已提交**，N97 已 pull + colcon build）；VM 反转重放 A/B PASS（09-07）；**实车验证完成（09-08 bag）** = 第一次接近停障有效 + **二次失效根因实锤**：新 goal 触发 global 障碍层整层清空（单帧抹除全部非静态 254）→ 盲区保留 mark 不可再生（结构性冲突）；**09-07 用户决策：机制精修收手**（成果保留不回滚，测试用高箱规避维持），主线转 3D 规控方向 — [方向决策 09-07](retrospect/2026-09-07_lowobstacle_pivot_decision.md)、[二次失效根因](retrospect/2026-09-08_lowobstacle_secondfail_clearevent.md)、[A/B 验收](retrospect/2026-09-07_lowobstacle_fixB_ab_acceptance.md)、[ringlaw/cleandata](retrospect/2026-09-07_lowobstacle_ringlaw_cleandata.md)、[09-06 撞箱](retrospect/2026-09-06_lowobstacle_fixB_crashbox.md)
...
- 全速版 Nav2 验证 — **暂缓（08-17 决策）**；切回前须先同步 nav2_params.yaml 膨胀 0.55→0.30 **及 obstacle_low_layer 独立 mark-only 结构**（09-07 修复，同 07-handover 警示）— [07-handover.md §四](07-handover.md)、[retrospect 09-07](retrospect/2026-09-07_lowobstacle_fixB_ab_acceptance.md)

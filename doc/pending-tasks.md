...
> 建档：2026-09-03 ｜ 关联：[07-handover.md](07-handover.md) §四（交接级遗留）、[02-progress.md](02-progress.md)（进度看板）
>
...
- Greenwave Monitor：N97 真机部署（VM 已验证）+ §6 四项待验证（launch 参数名/QoS 兼容/STALE 行为）— [greenwave-monitor-deploy.md](n97/greenwave-monitor-deploy.md)；**09-05 VM 侧运行实例被 pkill 误杀，恢复暂缓（用户定）** — [retrospect 09-05 §七](retrospect/2026-09-05_lowobstacle_fixB_vm_acceptance.md)
- N97 风扇 it87 驱动持久化 — **08-24 用户决策暂不做**，重启后需手动 modprobe + pwm — [n97info.md](n97/n97info.md)
- performance 治理器 systemd 固化 — 暂缓，已入每次开机手动流程 — [startup.md §一](startup.md)
...
- **~~近距丢黑块修复~~（低物链收手定论 09-08）**：修复 = `nav2_params_low.yaml` velodyne_low 拆独立 mark-only 层（**commit 180077c 已提交**，N97 已 pull + colcon build）；VM 反转重放 A/B PASS（09-08）；**实车验证完成（09-08 bag）** = 第一次接近停障有效 + **二次失效根因实锤**：新 goal 触发 global 障碍层整层清空（单帧抹除全部非静态 254）→ 盲区保留 mark 不可再生（结构性冲突）；**09-08 用户决策：机制精修收手**（成果保留不回滚，测试用高箱规避维持），主线转 3D 规控方向 — [方向决策 09-08](retrospect/2026-09-08_lowobstacle_pivot_decision.md)、[二次失效根因](retrospect/2026-09-08_lowobstacle_secondfail_clearevent.md)、[A/B 验收](retrospect/2026-09-08_lowobstacle_fixB_ab_acceptance.md)、[ringlaw/cleandata](retrospect/2026-09-08_lowobstacle_ringlaw_cleandata.md)、[09-06 撞箱](retrospect/2026-09-06_lowobstacle_fixB_crashbox.md)
...
- **costmap 刷新观感过慢（09-08 记，未查证）**：车遇"未探索区域"（unknown）边界会减速，但 global 刷新从 rviz 观感明显滞后——疑与上一条 0.64Hz 节流同源，查证时并入 — [09-08 clearevent 遗留](retrospect/2026-09-08_lowobstacle_secondfail_clearevent.md)（源文档已落位）
...
- 可选：VLP-16 vs MID-70 实机 A/B（触发条件已满足，未做；**装法与几何已定档** → 见 §⑥ 传感器加装）— [planning-control-roadmap.md §3.4](roadmaps/planning-control-roadmap.md)、[sensor-mounting-layout-survey.md](surveys/sensor-mounting-layout-survey.md)
...
- **~~WIP 泄漏处置~~（09-16 发生 → 09-16 闭环）**：`b2caad1` 误带入的 `sensor-mounting-layout-survey.md` 树行，已随该文档定稿 **commit `c6a9853` 提交并推送**（采纳"等文档写完一起提交"）→ 远端该行现有实际文件对应，404 消除 — [checker fix 档 §八](retrospect/2026-09-16_checker_codespan_fix.md)、[doc-engineering §七 协作边界](doc-engineering.md)（经验点 G4）
- **围栏内真实引用是否纳入闸门**：校验器代码跨度排除后，代码围栏内的真实引用（如 01-plan §8.2 状态块）脱离闸门——可加「围栏内白名单」一档，待用户定 — [checker fix 档 §五](retrospect/2026-09-16_checker_codespan_fix.md)
...
- **传感器加装：MID-70 + D435（阶段二，装法与几何已定档 09-16）**：MID-70 装 VLP-16 下方（离地 0.55~0.65m、前倾 15~20°）、D435 夹两雷达之间、拟走双雷达组合（roadmap 方案 b）；**待办** = ① 实车安装结构核算（三层叠装机械实现/供电余量/交换机口）② 外参标定（含 MID-70 倾角旋转）③ 实机验证或 A/B — [surveys/sensor-mounting-layout-survey.md](surveys/sensor-mounting-layout-survey.md)、[roadmap §三](roadmaps/planning-control-roadmap.md)

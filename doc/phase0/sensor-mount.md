...
| 参数 | 值 | 说明 |
|:-----|:---:|:-----|
| 底盘离地高度 | 12 cm | base_link 中心离地高度（08-24 实测更新，原 13cm） |
...
| 项目 | 值 | 说明 |
|:-----|:---|:-----|
| 安装位置 | **(-36, -3.5, +18.5) cm**（base_link 系） | ✅ 08-24 实车量测（钢卷尺）：距地 30~31cm − base_link 12cm = 18.5cm；静态 TF 当前为单位变换，平移待补齐（FAST-LIO 外参已用此实测值，见 [fastlio2-n97-deploy.md](../n97/fastlio2-n97-deploy.md) §三.1） |
...
| # | 问题 | 定案 |
|:--|:-----|:-----|
| 1 | IMU/雷达坐标的**基准系**？ | ✅ **base_link = 车体底盘正中心，离地 12cm**（08-24 复测更新，原 13cm）；base_footprint 已从 URDF 删除（TF2 双父冲突，无人使用） |
| 2 | 雷达高度 **65cm vs 77cm** 哪个对？ | ✅ **光学中心距地 77~78cm**（08-24 复测更新；08-06 曾定 69cm）；base_link→velodyne = **0.655m**（77.5−12）；URDF 已更新（原备份 .bak_20260806，本次 0.56→0.655 无备份） |
| 3 | G354 IMU 安装位置 (-30, 0, +20)cm | ✅ 08-24 实测 **(-36, -3.5, +18.5)cm**（距地 30~31cm − base_link 12cm）；静态 TF 平移仍未补（不影响 EKF 姿态/航向，见备注） |

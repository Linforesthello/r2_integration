...
> 最后更新: 2026-09-04（文档职能拆分：启动命令 → [startup.md](startup.md)，事件索引 → [retrospect/README.md](retrospect/README.md)）
> 当前进度: Phase 0 ✅ 100%｜Phase 1 ✅ 95%（08-12 yaw 方案①通过）｜Phase 2 ✅ 100%（KISS 建图）｜Phase 3 ⏳ 25%（Nav2 首闭环 08-15 + 降额过缝 08-17；A1 避障实测进行中，低物盲区断点已定位待重测）
...
| 3 | VLP16 + Nav2 导航 | ⏳ 25% | 首闭环 08-15、降额过缝 08-17（inflation 0.30）；A1 避障：08-25 首轮 + 低物盲区断点定位（09-04），全速验证暂缓保持降额 |
| 4/5 | 视觉 / 气动+编排 | ⏳ 0% | — |
...
**配置警示**：Nav2 降额版 `nav2_params_low.yaml` 膨胀 0.30；**全速版 `nav2_params.yaml` 仍是 0.55**，切回前须先同步（08-17 决策，见 [retrospect 08-17](retrospect/2026-08-17_nav2_initialpose_inflation_fix.md)）；N97 风扇调速**不持久化**，重启后需手动 modprobe（[retrospect 08-24](retrospect/2026-08-24_n97_fan_control.md)）。

---

## 四、交接级遗留（交接视角；全量待办入口见 [pending-tasks.md](pending-tasks.md)）

- [ ] **Nav2 全速验证**（暂缓 08-17，保持降额现状）：切 `nav2_params.yaml` 前先同步膨胀 0.55→0.30 再复测

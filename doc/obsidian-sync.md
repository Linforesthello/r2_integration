...
3. **跨子目录文件逐目录 cp，勿多源平铺**（2026-09-09 教训）：`cp f1 f2 f3 $DIR/` 会把所有文件平铺进 `$DIR` 根
   （丢目录结构）——minimal-loop/、phase1/、retrospect/ 等不同子目录的文件须按目录逐个 cp（或直接 rsync）；
   覆盖后 diff 若现"镜像独有"，先查是否自己上一步 cp 错放（见 §4 第 0 步）
...
独有 ≠ 真独有，处理前按顺序调查（2026-08-13 教训；2026-09-09 增补第 0 步）：

0. **是否自己上一步操作错放**：刚执行过 cp/rsync 且错放文件与权威源某子目录逐字节一致（`diff`/`cmp`）→
   直接 rm 错放副本 + 按正确子目录重放即可，不必走 1~3 步（2026-09-09 cp 多源平铺事故，见 §2 第 3 条）

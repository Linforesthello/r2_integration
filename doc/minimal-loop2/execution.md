...
> 关联: [plan.md](plan.md)（总计划：序列/时间窗/验收原则）｜ 状态: **A1 进行中**（08-25 首轮未满判据 → 低物盲区链 09-04/05/06 阻塞静态绕行判据；09-06 状态见 [1.5](#15-低物链路进展记录2026-09-06)）
...
### 1.5 低物链路进展记录（2026-09-06）

> 低物盲区（≤0.4m 矮物扫不到）链 = A1 判据 1「静态绕行无碰撞」的低物类阻塞项。进展全记录见
> [retrospect 09-06](../retrospect/2026-09-06_lowobstacle_fixB_crashbox.md)，此处只记 A1 卡相关状态。

- 09-04/05：断点定位（/scan 单环 ring8 结构盲区）→ 修法 B（local velodyne_low 低带源）VM 验收 PASS
- 09-06 实车：修法 B 生效确认（local 箱区 254 黑块，316 格/0.7m 窗）+ 修法 A（global 同源，e87a3f8）
  生效确认（global 远距刷出 254）——**但近距（<~1.59m）低带源全线掠顶无 mark + scan 同层 clear

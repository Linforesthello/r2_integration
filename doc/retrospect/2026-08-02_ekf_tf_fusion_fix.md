...
### 8-4 补充：修改版丢失 → 备份找回（✅ 已恢复）

**现象**：8-4 重启系统后 IMU 数据错乱（"左右反"、姿态异常）——但 8-3 明明验证过正常。

**根因**：8-3 晚恢复 git 混乱时，`mv r2_integration → /tmp/r2_git_backup/` 把
**8-3 修改版（mount_axes 轴映射）源码移走**，`r2_integration_old`（8-3 早的原始版）
改回原名 → 之后启动的是**无 mount_axes 的原始版**。launch 传未声明参数被 rclpy
**静默拒绝**（不报错）→ 驱动按默认标准朝向处理 → 数据错乱。
且 /tmp 备份随后被清理，修改版一度只剩对话记录。

**恢复**：发现 `~/Lin_workspace/r2_integration_old_bak`（8-3 17:58 用户备份）内含
修改版 imu_node.py（mount_axes=5）与 launch（mount_axes=2）→ 拷回 VM + N97 → 重编译 → 重启验证。

**验证**：静止 yaw≈0；左转 → yaw 增大（方向正确）；EKF 重启后 RViz 观测正常。

**教训**：
1. **改完代码立即同步两端 + 确认备份**：8-3 修改后未在 VM 留存完整备份（VM 文件随后被还原）
2. **launch 传未声明参数会被静默拒绝**（rclpy）——驱动"没反应"时先查参数是否声明
3. **/tmp 备份不可靠**（重启即清）——重要备份放 `~/` 下
4. **`r2_integration_old_bak` 是有效备份点**（8-3 17:58），恢复 git 混乱时该目录内容为修改版

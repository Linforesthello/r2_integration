...
- **顺序**（每步有判据，达标即走）：
  0. **前置：08-25 四问题解决方向全景核对**（对照清单 + 本阶段处置，全景表见 [recruitment-learning-plan-review.md §3.5](recruitment-learning-plan-review.md)，含来源）：
     ...
     - ② 避让不及时（根因已定论：MPPI 空间前瞻 ≈0.38m，感知无责）→ 同 08-25 基线跑 A1，**方案① 调权留阶段二**（第 2 步下注同）
  ...
  2. **静态绕行 ×3**（同 08-25 参数，不新增变量——[5.7ter 方案① critic 权重调节](planning-control-roadmap.md)是**参数变更**，会污染与 08-25 基线的对比，整体留到阶段二）
...
1. **双版简历增量更新**（08-23 版 → v2）：并入 08-24 FAST-LIO2 验证（量化数字）、08-25 避障一轮 + 排障深度、RL 训练素材（如已有曲线图）、LocoWiki 学习（如写"体系化学习"可提）；按目标 JD 双线分别微调
...
- A2、B1（RL 环境）、B2（explore/D435）、传感器 A/B、5.7ter 方案① —— 全部后置
- 新功能 / 新项目 / 系统学习新体系
...
### 5.1 双线主线项目（用户 09-02 定双线并行；每线只深一个交付）

| 线 | 交付候选（择一深化） | 依据 |
|:---|:---|:---|
| **规划控制线** | R2 收尾序列：A2（FAST-LIO2 TF 桥 + 建图源，卡见 [execution.md](minimal-loop2/execution.md) A2）→ 5.7ter 方案① 实车验证 →（有余力）车构型 RL 局部避障 v1（Isaac Lab + R2 实车 A/B vs MPPI，素材见 planning-control-roadmap §6.0/6.2/6.8） | [planning-control-roadmap.md](planning-control-roadmap.md) §6 |
| **运动控制线** | Go2/G1 地形扩展（域随机化 → 蒸馏完整复现曲线）或 机械臂 SO-101 + LeRobot 数据闭环（预算项）择一 | [motion-control-roadmap.md](motion-control-roadmap.md) §七（秋招后项） |

> 双线并行的防浅尝纪律（呼应全局反"多线浅尝"）：**周粒度单主线**（一周内只切一次线，不日内混切）；每项 1~2 周一个小闭环（复现出曲线/出视频/出数据）；沿用 minimal-loop2「判据卡 + 达标即走」。
...
| 时间 | 里程碑 | 验收 |
|:---|:---|:---|
| 2026-10 底 | R2 收尾 + 双线各第一个小闭环 | A2 判据 3/3 + 各线首次出数据 |
| 2026-11 底 | 复现项目 1 完成（规划或运动线） | 曲线/视频 + 文档留档（带来源） |
